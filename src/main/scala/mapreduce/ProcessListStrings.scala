...
object ProcessListStrings {
   ...
   *
   * @param filename
   * @return (Title, Body, List[Reference])
   */
  def getFileParts(filename: String): (String, String, List[String]) = {

    val parseResult = ViquipediaParse.parseViquipediaFile(filename)
