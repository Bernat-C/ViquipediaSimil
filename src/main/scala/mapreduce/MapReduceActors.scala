...
class MapReduce[K1,V1,K2,V2,V3](
                                 ...
                                 reducing:(K2,List[V2])=> (K2,V3), nmap: Int, nred: Int) extends Actor {
  ...
  def receive: Receive = {
    ...
    case fromMapper(list_clau_valor:List[(K2,V2)]) =>
      ...
      if (mappersPendents==0)
      {
        ...
        nreducers = this.nreducers
        reducersPendents = dict.size // actualitzem els reducers pendents
        ...
        for ((i,(key:K2, lvalue:List[V2])) <-  (0 until dict.size) zip dict) {
